...
if __name__ == "__main__":
    ...
    dfs = pd.read_excel(
        "C:/Xiaomi Cloud/Desktop/DWD.xlsm",sheet_name=None,  header=None
    )
    for sheet in dfs:
        ...
        script = (
            f"""
    WITH STOCKDATA AS (
        SELECT * FROM {tableName} t WHERE EXISTS (SELECT 1 FROM { originalTableName} o WHERE t.ds = o.{partitionField} )
    )
    ...
    ON 
        {str_keys} 
    WHERE 
        {pk_fields[0]} IS NULL  -- 请将 some_field 替换为实际字段名
    UNION ALL
    SELECT
        {str_fields},
